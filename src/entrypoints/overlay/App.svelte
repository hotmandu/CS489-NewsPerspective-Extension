<script lang="ts">
  ...
  const overlay = async () => {
        ...
        function getBiasLevel(score: number) {
        ...
        function getSubjectivityLevel(score: number) {
        ...
        function highlightText(text: string, color: string, type: string) {
        ...
        function addHighlights(data: ApiAnalyzeResult) {
          ...
          if (analysis.emotional_language) {
            analysis.emotional_language.forEach((item) => {
              ...
              const type = `Emotional Language (Intensity: ${item.intensity.toFixed(
                2
              )})`;
        ...
        function createAnalysisDisplay(data: ApiAnalyzeResult) {
          const analysis = data.analysis;
          ...
          const sentimentScore =
            ...
            <div style="flex: 1; overflow-y: auto; padding: 15px 0;">
              <div style="font-weight: bold; margin-bottom: 10px;">GPT Article Comparison</div>
        ...
        try {
          ...
          const data: ApiAnalyzeResult = await response.json();
  ...
  onMount(() => {
    ...
    overlay().then((res) => {
      if (res.success) {
        ...
        loadingState = JSON.stringify(res.error);
      }
    }).catch((e) => {
      loadingState = JSON.stringify(e);
    })
  });
